...
func initRole() {
	...
	// custom role
	roleMap := redis.HGetAll(AI_ROLE_KEY)
	if roleMap != nil && len(roleMap) == 0 {
		return
	}
...
func setRoleCommand(ctx *robot.Ctx, msg string, command string) {
	...
	case "当前角色":
		var role string
		if val, ok := roomCtx.Load(ctx.Event.FromUniqueID + "_" + ctx.Event.FromWxId); ok {
			role = val.(Room).role
		}
		if role == "" {
	...
	case "创建角色":
		...
		desc := matched[2]
		if redis.HExists(AI_ROLE_KEY, role) {
	...
	case "删除角色":
		...
		if !redis.HDel(AI_ROLE_KEY, role) {
	...
	case "切换角色":
		...
		roomCtx.Store(chatRoom.id, chatRoom)
